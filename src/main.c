...
static void rebuild_selectables(void)
{
    ...
    qsort(&g_selectable->positions[0], g_selectable->count,
          sizeof(position_t), cmp_pos);
...
static void undo()
{
    ...
    for (i = 0; i < 2; ++i) {
        board_set(&g_board, &undo_stack.positions[undo_stack.count - 1],
                  undo_stack.chips[undo_stack.count - 1]);
        --undo_stack.count;
    }
...
#define IMG_HEIGHT (79)
    ...
    if (IMG_WIDTH * col_count * screen_height >
        screen_width * IMG_HEIGHT * row_count) {
...
static void draw_chip(const position_t * pos, chip_t chip)
{
    ...
    for (i = 0; i < 3; ++i) {
        DrawLine(r.x - 1 - i, r.y - 1 - i, r.x - 1 - i,
                 r.y - 1 - i + r.h - 1, LGRAY);
        DrawLine(r.x - 1 - i, r.y - 1 - i, r.x - 1 - i + r.w - 1,
                 r.y - 1 - i, LGRAY);
    }
    ...
    StretchBitmap(r.x + 1, r.y + 1, r.w - 2, r.h - 2,
                  (ibitmap *) bitmaps[chip], 0);
    ...
    /* status bar */
    {
...

        {
            ...
            for (i = 0; i < g_selectable->count - 1; ++i) {
                const chip_t chip1 =
                    board_get(&g_board, &g_selectable->positions[i]);
                for (j = i + 1; j < g_selectable->count; ++j) {
                    const chip_t chip2 =
                        board_get(&g_board, &g_selectable->positions[j]);
                    if (fits(chip1, chip2))
                        ++pairs;
                }
            }
            ...
            DrawTextRect(r.x, r.y, r.w, r.h, buffer,
                         ALIGN_FIT | ALIGN_LEFT);
        }

        DrawTextRect(r.x, r.y, r.w, r.h, (char *) get_message(MSG_HELP),
                     ALIGN_FIT | ALIGN_RIGHT);
    }
}
...
static int move_up(void)
{
    ...
    for (i = 0; i < g_selectable->count; ++i)
        if (i != caret_pos) {
            ...
            const int dist =
                abs(caret_row - pos_row) * MAX_COL_COUNT + abs(caret->x -
                                                               pos->x);
            if (dist < min_dist) {
                new_pos = i;
                min_dist = dist;
...
static int pair_exists(board_t * board)
{
    ...
    for (i = 0; i < g_selectable->count - 1; ++i) {
        ...
        for (j = i + 1; j < g_selectable->count; ++j) {
            const chip_t chip2 =
                board_get(board, &g_selectable->positions[j]);
            if (fits(chip1, chip2))
                return 1;
        }
    }
...
static void select_cell(void)
{
    ...
    const chip_t chip1 = selection_pos >= 0 ? board_get(&g_board,
                                                        &g_selectable->positions
                                                        [selection_pos]) :
        0;
    const chip_t chip2 =
    ...
    if (chip1 != 0 && fits(chip1, chip2)) {
        undo_stack.positions[undo_stack.count] =
            g_selectable->positions[selection_pos];
        undo_stack.chips[undo_stack.count] = chip1;
        ++undo_stack.count;
        undo_stack.positions[undo_stack.count] =
            g_selectable->positions[caret_pos];
    ...
    case EVT_POINTERDOWN:
        {
            ...
            point_change_orientation(par1, par2, GetOrientation(), &rx,
                                     &ry);
            int max=0;
            for (i = 0; i < g_selectable->count; ++i) {
                struct rect r;
                ...
                if (point_in_rect(rx, ry, &r)) {
                    ...
                    main_repaint();
                    cell_rect(&g_selectable->positions[prev_caret_pos],
                              &prev_r);
                    PartialUpdate(prev_r.x, prev_r.y, prev_r.w, prev_r.h);

                    select_cell();
                    return 1;
                }else if(r.x>max)max=r.x;
            }
            if(rx>max)show_game_menu();
        }
        break;
    }
    return 0;
...
static void menu_handler(int index)
{
    ...
    case MSG_CHANGE_ORIENTATION:
        ...
        ClearScreen();
        StretchBitmap(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                      (ibitmap *) & background, 0);
        FullUpdate();
        show_popup(&background, MSG_NONE, main_menu, menu_handler);
        break;
...
static int load_game(void)
{
    ...
    for (i = 0; i < undo_stack.count; ++i)
        fscanf(f, "%d %d %d %d\n",
               ...
               &undo_stack.positions[i].k, &undo_stack.chips[i]);
